...
public class VendedorDaoJDBC implements VendedorDao {
	...
	public void insert(Vendedor obj) {
		...
		try {
			st = conn.prepareStatement(
					...
					+ "(nome, email, dt_aniversario, salario_base, departamento_id) "
					+ "VALUES "
					+ "(?, ?, ?, ?, ?)",
			...
			st.setDate(3, new java.sql.Date(obj.getDtAniversario().getTime()));
	...
	public void update(Vendedor obj) {
		...
		try {
			st = conn.prepareStatement(
					"UPDATE vendedor "
					+ "SET nome = ?, email = ?, dt_aniversario = ?, salario_base = ?, departamento_id = ? "
					+ "WHERE id = ?");
	...
	private Vendedor instantiateVendedor(ResultSet rs, Departamento dep) throws SQLException {
		...
		obj.setSalarioBase(rs.getDouble("salario_base"));
		obj.setDtAniversario(rs.getDate("dt_aniversario"));
